...
async function main() {
  ...
  for (const sheet of workbook.worksheets) {
    ...
    sheet.eachRow((row, rowNumber) => {
      ...
      // Disk is the truth for audio existence; _3 (TTS) is guaranteed for every word.
  ...
  // --- "Needs disambiguation note" report ---
  ...
  const lines = [
    ...
    '> `disambiguation-notes.json`. Each flagged group needs a human-written note',
    '> before it silently ships empty.',
    '',
